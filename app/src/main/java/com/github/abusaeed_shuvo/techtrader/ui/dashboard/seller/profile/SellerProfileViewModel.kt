...
class SellerProfileViewModel @Inject constructor(
	private val sellerService: SellerRepository,
	...
	fun getUserById(userId: String) {
		...
		sellerService.getUserDataById(userId).addOnSuccessListener { documentSnapshot ->
	...
	fun updateUserDetails() {
		...
		if (newImageUri?.toString() != user.profileImageLink) {
			sellerService.uploadUserImage(newImageUri!!, user.id)
	...
	private fun updateUserInDb(userEntity: UserEntity) {
		authService.updateUserProfile(userEntity)
